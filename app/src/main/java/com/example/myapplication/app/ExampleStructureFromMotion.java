...
import org.ddogleg.struct.GrowQueue_I32;
/**
 * Created by nuonuomisu on 15/7/14.
 */
...
public class ExampleStructureFromMotion {
    ...
    public void process(IntrinsicParameters intrinsic , List<Bitmap> colorImages ) {
        ...
        private void detectFeatures(Bitmap colorImage,
            ...
            ImageFloat32 image = new ImageFloat32(colorImage.getWidth(),colorImage.getHeight());
            image = ConvertBitmap.bitmapToGray(colorImage, image, null);
        ...
        private Feature3D lookupFeature(List<Feature3D> features, int frameIndex, Point2D_F64 pixel) {
            for (int i = 0; i < features.size(); i++) {
                ...
                for (int j = 0; j < t.frame.getSize(); j++) {
